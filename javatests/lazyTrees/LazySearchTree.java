 ...
 * Node in the tree as "deleted", but not removing it from the tree until
 * later.
 *
 * @param <E>               Generic parameter that only accepts data items
 *                          that implement Comparable<E>
 */
public class LazySearchTree<E extends Comparable<? super E>>
    ...
    /**
     * Accessor method for the current size of deleted and undeleted nodes in
     * the tree
    ...
    /**
     * Public method that finds the minimum value in the tree by parsing through
     * the left-most children from the top of the tree.
    ...
    /**
     * Public method that finds the maximum value in the tree by parsing through
     * the right-most children from the top of the tree
     ...
     * @return              Generic data of Node that contains the requested
     *                      data
     */
    public E find(E x) {
    ...
    /**
     * Public method that determines if the tree contains a user requested
     * Generic data item
     * @param x             Generic data item
     * @return              Boolean value of whether the data item is in the
     *                      tree or not
     */
     ...
     * @param x             Generic data item
     * @return              Boolean value of whether the data item was
    ...
    /**
     * Cloning function that performs a clone of the original tree and copies
     * it into a new sub tree
     * @return              Returns the cloned binary tree
     * @throws CloneNotSupportedException       Exception thrown when clone
     *                                          is not possible
     */
    ...
    /**
     * Logic of public findMax() that determines the maximum value in the
     * tree. Passes over nodes that were soft removed from the tree
     * @param root          Passed in LazySTNode node
     * @return              Recursive call to itself
     */
    ...
    /**
     * Logic of public insert() that inserts a LazySTNode with a Generic data
     * item. Determines if a Node has been soft removed and sets the deleted
     * attribute back to false if reinserting value.
     * @param root          Passed in LazySTNode node
     * @param x             Generic data item
    ...
    /**
     * Private helper class for insert() that determines if the first keyword
     * of the passed in root's data and the generic data are the same
     * @param root          Passed in LazySTNode node
     * @param x             Generic data item
     * @return              Boolean value of whether they're both the same
     */
    private boolean reInserted(LazySTNode root, E x) {
        String rootData = root.data.toString().substring(0,
                root.data.toString().indexOf(":"));
        String genericData = x.toString().substring(0,
                root.data.toString().indexOf(":"));
     ...
     * physically remove the node from the tree, but simply changes the
     * node's deleted attribute to true
     * @param root          Passed in LazySTNode node
     * @param x             Generic data item
     */
    ...
    /**
     * Logic of public method traverseHard() that recursively traverses the
     * nodes that have been deleted and not deleted
     * @param func          Generic function object
    ...
    /**
     * Logic of public method traverseSoft() that recursively traverses the
    ...
    /**
     * Logic of public method find(). Passes over any nodes that have been
     * soft removed from the tree
     * @param root          Passed in LazySTNode node
     * @param x             Generic data item
     * @return              LazySTNode node that contains the requested
     *                      generic data item
     */
     ...
     * @param root          Passed in LazySTNode node
     * @return              New LazySTNode that is a copy of the original
     *                      at location
     */
    ...
    /**
     * Logic of public method findHeight() that determines the height of the
     * tree
     * @param treeNode      Passed in LazySTNode node
    ...
    /**
     *  An object of LazySTNode is similar to a Node in an ADT, except it
     *  includes data attributes for the left child, right child, data type
     *  of itself, whether it has been deleted or not, and the root of the
     *  node. Properties of a Binary Search Tree (BST) node
     */
